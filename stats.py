...
def load_events_data(type = 'rt'):
    with open('events_{}.csv'.format(type)) as csvfile:
        ...
        for r in reader:
            ...
            if (type == 'rt'):
                rt_events_by_name[formatted_name].append(event)
                rt_events_by_war_id[event['warid']].append(event)
            else:
                ct_events_by_name[formatted_name].append(event)
                ct_events_by_war_id[event['warid']].append(event)
